...
{
	ComponentID id = state->nbSprites++;
	Assert(id < WORLD_SIZE);

	Sprite* result      = &state->sprites[id];
...
ComponentID CreateTimeSprite(uint32 nbBitmaps, Bitmap** bitmaps, real32 stepTime, bool32 active)
{
	ComponentID id = state->nbSprites++;
	Assert(id < WORLD_SIZE);
...
{
	ComponentID id = state->nbPatterns++;
	Assert(id < WORLD_SIZE);

	RenderingPattern* result = &state->patterns[id];
	result->patternType      = RenderingPattern_Unique;
	result->unique           = state->sprites + sprite;
...
internal void FlushRenderQueue(RenderQueue* renderQueue)
{
	// Avoid state changes as much as possible
	std::sort(renderQueue->begin(), renderQueue->end(), [](const Mesh& a, const Mesh& b) {
		if (a.program < b.program) return true;
		if (a.program > b.program) return false;

		if (a.renderMode < b.renderMode) return true;
	...
	while (start < queueSize)
	{
		...
		while (end < queueSize && (*renderQueue)[end].renderMode == currMode &&
		       (*renderQueue)[end].texture == currTexture && (*renderQueue)[end].color == currColor)
		{
			++end;
		}
		...
		z::mat3 projMatrix = GetProjectionMatrix(currMode);
...
void FlushRenderQueue()
{
	...
	FlushRenderQueue(&g_defaultRenderQueue);

	RenderParticles();

	glDisable(GL_DEPTH_TEST);

	FlushRenderQueue(&g_uiRenderQueue);
	FlushRenderQueue(&g_debugRenderQueue);
}

void RenderPattern(ComponentID patternId, Transform* transform, z::vec2 size)
{
	RenderingPattern* pattern = state->patterns + patternId;
	switch (pattern->patternType)
	{
		...
		{
			RenderFillPattern(patternId, transform, size);
		}
		break;
...
void RenderFillPattern(ComponentID patternId, Transform* transform, z::vec2 size)
{
	RenderingPattern* pattern = state->patterns + patternId;
...
void RenderParticles()
{
	...
	z::mat3 projMatrix = GetProjectionMatrix(RenderMode_World);
	...
	if (particleCount > 0) Log(Log_Info, "Rendering %i particles", particleCount);
	...
	glBufferData(GL_ARRAY_BUFFER,
	             positionsSizes.size() * sizeof(z::vec4),
	             positionsSizes.data(),
	             GL_STREAM_DRAW);
	...
	glBindTexture(GL_TEXTURE_2D, state->particleBitmap.texture);
...
z::mat3 GetProjectionMatrix(RenderMode renderMode)
{
	...
	switch (renderMode)
	{
		...
		{
			result[0][0] = 2 / state->viewportSize.x;
			result[0][2] = -1;
			result[1][1] = -2 / state->viewportSize.y;
			result[1][2] = 1;
		}
		break;
		...
		{
			result[0][0] = 2 / state->worldSize.x;
			result[1][1] = 2 / state->worldSize.y;
		}
		break;
...
RenderingPattern* GetPattern(Entity* e)
{
	return state->patterns + e->pattern;
}